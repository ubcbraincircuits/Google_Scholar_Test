...
# pull names takes the input file name and appends an output and thus creates a new empty file for the 
# requested information to be put in.
# 
# inputs: fname (string)
# outputs: new_name (string)
...
# pull names takes the input file .csv and pulls the names in the order and sorts them into an array to then be passed 
# to other functions
# 
# inputs: fname (string)
...
def index_method(fname,author_names):
  ...
  with open(fname, mode='w', encoding ="utf-8") as csv_file:
    ...
    for author_string in author_names:
      ...
      row = [author_string[0],hindex,hindex5,i10index,i10index5]
      csv_writer.writerow(row)
...
def both_method(fname,author_names):
